...
@param[in] string_arguments key argument for which the key field is a pointer to
SV_Str_view.
...
function also only iterates read-only over the range provided so it is safe to
provide non-null terminated data to this function (even though the SV_Str_view
library says this is not recommended). Just ensure the SV_Str_view is the
correct length when provided. SV_Str_view assumes that the position at the
length of the input string may or may not be the null terminator and this
function does not use any methods that need or care what is at that position. */
